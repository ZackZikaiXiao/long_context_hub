...
def parse_config():
    ...
    parser.add_argument('--base_model', type=str, default="/home/zikaixiao/zikaixiao/LongLoRA-main/TinyLlama-1.1B-16k-ft-sft")
    ...
    parser.add_argument('--vallina_model', type=bool, default=False, help='use the official (non-finetune) model')
...
def passkey_retrieval_test(model, tokenizer, device, use_cache, n_garbage=60000, seed=666):
...
def passkey_retrieval_in_test(model, tokenizer, device, use_cache, n_garbage=60000, seed=666):
    prompt, answer = generate_prompt_landmark(n_garbage, seed)
    input_ids = tokenizer(prompt, return_tensors="pt").input_ids
    input_ids = input_ids.to(device)
    len_token = input_ids.shape[-1]
    ...
    answer_ids = tokenizer(answer, return_tensors="pt").input_ids[:, 1:] # drop BOS
    generation_output = model.generate(
        input_ids=input_ids, max_new_tokens=30, num_beams=1, use_cache=True
    )
...
def main(args):
    ...
    # if args.vallina_model:
    #     pass
    # else:
    #     if args.flash_attn:
    #         replace_llama_attn(use_full=True)

    if not args.vallina_model:
    ...
    for i in range(total_test_points):
        ...
        passed_tests = 0
        total_tokens = 0
        # if n_garbage < (int(3.75 * (total_test_points + 1) * args.interval // 1024 * 1024))/2.5:
        #     continue
        for i in range(args.num_tests):
            is_correct, len_tokens = passkey_retrieval_in_test(model, tokenizer, device, use_cache=not args.flash_attn, n_garbage=n_garbage, seed=i)
