...
parser.add_argument("--tokenizer_path", type=str, default="/home/zikaixiao/zikaixiao/LongLoRA-main/models/llama2-7B-4k", help='path to the tokenizer model')
...
# Load Tokenizer
TOKENIZER = HFTokenizer(model_path=args.tokenizer_path)
...
def sys_vartrack_w_noise_random(num_samples: int, max_seq_length: int, incremental: int = 10, 
    ...
    if add_fewshot and (icl_example is not None):
        icl_example_out = ' '.join(icl_example['output'])
        icl_example = icl_example['instruction'] + " " + icl_example_out + '\n\n'
...
def main():   
    ...
    save_file = "./variable_tracking_32k.jsonl"
