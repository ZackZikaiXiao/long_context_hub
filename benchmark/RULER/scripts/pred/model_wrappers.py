...
class HuggingFaceModel:
    def __init__(self, name_or_path: str, **generation_kwargs) -> None:
        ...
        self.model = LlamaForCausalLM.from_pretrained(name_or_path, trust_remote_code=True, device_map="auto", torch_dtype=torch.bfloat16,)  
    ...
    def process_batch(self, prompts: List[str], **kwargs) -> List[dict]:
        if self.pipeline is None:
            ...
            for i in range(len(generated_ids)):
                input_length = len(inputs[i].ids)  # 获取输入的长度
                response = generated_ids[i][input_length:]  # 从生成的 ID 中移除输入部分
