...
def forward_flashattn(
    ...
    # 一个针对可变长度序列优化的Flash Attention版本，它处理重新排列和重塑后的qkv。
...
# 在处理自注意力（self-attention）时，通常会用到一个叫做“attention mask”的结构，它用于指示哪些位置的单元应该接收注意力。这在处理不等长序列或者希望某些特定位置不被注意到的场景下特别有用。
# 在原始的LLaMA模型中，_prepare_decoder_attention_mask函数的作用是根据提供的attention_mask、input_shape、inputs_embeds和past_key_values_length参数，准备和调整用于解码器的注意力掩码。这个过程通常涉及到将attention_mask扩展或修改，使其适应于模型当前的状态和输入长度。
# 然而，当引入“Flash Attention”作为注意力机制的优化版本时，存在一个特定的需求：Flash Attention要求用于查询（query）、键（key）和值（value）的注意力掩码必须与key_padding_mask相同。key_padding_mask是一个二维的张量，其作用是在计算注意力权重时忽略（或遮蔽）序列中的某些位置，这通常用于处理填充（padded）的序列数据。
# 因此，_prepare_decoder_attention_mask函数的定义被修改为直接返回attention_mask，而不进行任何转换或调整。这样做是为了确保Flash Attention能正确地接收和处理输入序列中的掩码信息，而不受到原始LLaMA模型中可能存在的任何注意力掩码转换逻辑的影响。
# 简而言之，这一修改是为了确保在使用Flash Attention时，能够正确地处理那些需要被忽略或遮蔽的序列位置，使得注意力机制只关注那些真正重要的部分。通过这样做，可以提高模型处理不等长序列的灵活性和效率。
