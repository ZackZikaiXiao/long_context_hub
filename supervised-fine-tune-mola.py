...
from src_mola.mola_mapping_hacked import get_peft_model
from src_mola.mola_lora_hacked import LoraConfig
from src_mola.mola_peft_model_hacked import set_peft_model_state_dict_moe
from src_mola.mola_trainer_hacked import Trainer

from transformers import LlamaTokenizer, AutoConfig
from src_mola.mola_modeling_llama_hacked import LlamaForCausalLM_d
...
def jload(f, mode="r"):
    ...
    num_samples = int(len(jdict) * 0.001)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    output_dir: str = field(default="./TinyLlama-1.1B-16k-ft-sft-mola")
    ...
    # deepspeed: str = field(default="ds_configs/stage3.json")      ####################### 并行是有的
    bf16: bool = field(default=True)  # 注意，`bf16` 和 `tf32` 参数需要根据 transformers 的版本和支持进行调整
    # tf32: bool = field(default=True)
    # max_steps: int = field(default=1000)

    # lora hyperparams
    lora_r: str = field(default="8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8")
    lora_alpha: int = field(default=16)
    lora_dropout: float = field(default=0.05)
    lora_target_modules: str = field(default="q_proj,v_proj,k_proj,o_proj,gate_proj,down_proj,up_proj")

    # mola hyperparams
    number_experts: str = field(default="2,2,2,2,2,2,2,2,4,4,4,4,6,6,6,6,8,8,8,8,10,10")
    top_k: str = field(default="2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2")
    obalance: bool = field(default=True)
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, filter_mode: str):
        ...
        # list_data_dict = load_and_resample_dataset(data_path, tokenizer)
...
def train():
    ...
    if isinstance(training_args.lora_r, str):
        training_args.lora_r = training_args.lora_r.split(",")
        training_args.number_experts = training_args.number_experts.split(",")
        training_args.top_k = training_args.top_k.split(",")
        training_args.lora_target_modules = training_args.lora_target_modules.split(",")

        training_args.lora_r = [int(lr) for lr in training_args.lora_r]
        training_args.number_experts = [int(ne) for ne in training_args.number_experts]
        training_args.top_k = [int(tk) for tk in training_args.top_k]
        training_args.lora_target_modules = [str(ltm) for ltm in training_args.lora_target_modules]
    else:
        training_args.lora_r = [int(lr) for lr in training_args.lora_r]
        training_args.number_experts = [int(ne) for ne in training_args.number_experts]
        training_args.top_k = [int(tk) for tk in training_args.top_k]
    ...
    config.lora_target_modules = training_args.lora_target_modules
    ...
    model.get_new_parameters(training_args.number_experts, training_args.top_k, training_args.obalance)
    ...
    model = get_peft_model(model, config, number_experts=training_args.number_experts, top_k=training_args.top_k)
