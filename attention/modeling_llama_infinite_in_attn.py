...
class LlamaInfiniAttention(LlamaAttention):
    def __init__(
        ...
        # self.memory_gate = nn.Parameter(torch.randn(1, self.num_heads, 1, 1))
        self.memory_gate = nn.Parameter(torch.full((1, self.num_heads, 1, 1), 0.0))
        self.count = 0
        # self.memory_gate = 1
        # self.memory_gate = torch.full((1, self.num_heads, 1, 1), 1e-8, requires_grad=True)
    ...
    def forward(
        ...
        **kwargs,
        ...
        batch_size, seq_len, hidden_dim = hidden_states.shape   # batch_size=1, seq_len=4987, hidden_dim=2048 (llama default) 
        n_seq, rem = divmod(seq_len, segment_len)   # n_seq=2, rem=891, 三段：[2048, 2048, 891]
        segments = []
        position_ids_range = []
        out = []    # 为了最后输出的concatenate
        # if rem != 0:
        #     raise ValueError(
        ...
        for ix in range(n_seq):
            ix_lo = ix * segment_len
            ix_hi = ix_lo + segment_len
            position_ids_range.append([ix_lo, ix_hi])

        # 如果 rem 不为 0，将最后的 rem 部分作为一个单独的段
        if rem != 0:
            position_ids_range.append([n_seq * segment_len, seq_len])

        # 在这个for循环中，我们直接从 hidden_states 中取得对应的段
        for ix_lo, ix_hi in position_ids_range:
            segment = hidden_states[:, ix_lo:ix_hi, :]
        ...
        # # 如果 rem 不为 0，将最后的 rem 部分作为一个单独的段
        # if rem != 0:
        #     segment = hidden_states[:, n_seq * segment_len:, :]
        #     segments.append(segment)
        #     position_ids_range.append([n_seq * segment_len, seq_len])

        # for seg_id in range(len(segments)):
        #     segment = segments[seg_id]
            # segment, (bs, length, hidden_dim) = (1, 2048, 2048)
            query_states = self.q_proj(segment) # q_proj; 2048 -> 2048
            key_states = self.k_proj(segment)   # k_proj; 2048 -> 256
            value_states = self.v_proj(segment) # v_proj; 2048 -> 256
            # query_states:(1, 2048, 2048), key_states:(1,2048,256), value_states:(1,2048,256)
            # Assuming the presence of batch size and dimension handling as before
            bsz, q_len, _ = segment.size()  # q_len == self.segment_size
            query_states = query_states.view(   # query_states: (bs, num_heads, length, head_dim)=(1, 32, 2048, 64)
                bsz, q_len, self.num_heads, self.head_dim
            ).transpose(1, 2)
            key_states = key_states.view(       # (1, 4, 2048, 64)
                bsz, q_len, self.num_key_value_heads, self.head_dim
            ).transpose(1, 2)
            value_states = value_states.view(   # (1, 4, 2048, 64)
                bsz, q_len, self.num_key_value_heads, self.head_dim
            ).transpose(1, 2)
            ...
            if no_memory_update:
                ...
                memory_output = self._retrieve_from_memory(                 # memory_output (1, 32, 2048, 64)，和query_states维度一致
                    query_states,
            ...
            updated_memory, updated_norm_term = self._update_memory(    # updated_memory (1, 4, 64, 64)，把kv的length从2048压缩到了64
                key_states,                                             # updated_norm_term (1, 4, 1, 64)
                value_states,
                memory.get(self.layer_idx, None) if memory is not None else None,
                norm_term.get(self.layer_idx, None) if norm_term is not None else None,
            )
            ...
            # cos, sin = self.rotary_emb(value_states, position_ids)
            # cos, sin = self.rotary_emb(value_states, position_ids[:, position_ids_range[seg_id][0]:position_ids_range[seg_id][1]])
            cos, sin = self.rotary_emb(value_states, position_ids[:, ix_lo:ix_hi])
            ...
            if attention_mask is not None:
                ...
                causal_mask = causal_mask[:, :, : key_states.shape[-2], : key_states.shape[-2]]
            ...
            if memory_output is None:
                ...
                combined_output = (
                    F.sigmoid(self.memory_gate) * memory_output
                    + (1 - F.sigmoid(self.memory_gate)) * attn_output
                )
                # combined_output = (memory_output + attn_output)

            # if self.count % 100 == 0:
            #     print(self.memory_gate[0].view(-1))
            #     self.count = 0
            # self.count += 1
            # Prepare output for this segment
            combined_output = combined_output.transpose(1, 2).contiguous()
            combined_output = combined_output.view(bsz, q_len, self.hidden_size)    # (1, 2048, 2048)
            out.append(self.o_proj(combined_output))                                # o_proj: 2048 -> 2048

            if no_memory_update:
                memory = None
                norm_term = None
        if torch.isnan(attn_output).float().mean().item() * 100 != 0:
            print(torch.isnan(attn_output).float().mean().item() * 100)
        return (
    ...
    def _retrieve_from_memory(self, query_states, memory, norm_term):
        ...
        memory_output = torch.matmul(       # 这个输出非常大，具体是memory本身很大
            # GQA
            query_states,
        ...
        norm_term_broadcastable = torch.matmul(     # norm非常非常非常大
            query_states,
            # GQA
    ...
    def _update_memory(self, key_states, value_states, memory, norm_term):
        ...
        if memory is not None:
            # memory = memory + torch.matmul(key_states.transpose(-2, -1), value_states)  # linear
            memory = memory + torch.matmul(key_states.transpose(-2, -1), 
                                           (value_states - torch.matmul(key_states, memory) / torch.matmul(key_states, norm_term.transpose(-2, -1))))  # delta
        else:
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        config._attn_implementation = "infinite"
