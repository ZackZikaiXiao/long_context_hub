        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            if calibration_enable:
                if first_iteration:
                    ...
                    ########################################################################
                    # 1. 设置模型参数
                    model_kwargs["chunk_len"] = 512
                    model_kwargs["init_token_len"] = 4
                    model_kwargs["query_len"] = 400
                    chunk_len = model_kwargs["chunk_len"]
                    query_len = model_kwargs["query_len"]
                    init_token_len = model_kwargs["init_token_len"]

                    
                    # 定义分块处理的范围
                    chunks = range(init_token_len, model_inputs["attention_mask"].shape[1] - query_len, chunk_len)
                    ...
                    if first_iteration:
                        # 3. 处理每个块
                        for start in chunks:
                            end = min(start + chunk_len, model_inputs["attention_mask"].shape[1]-query_len)
                            input_ids_chunk = input_ids.clone()
                            model_inputs_chunk = self.prepare_inputs_for_generation(input_ids_chunk, **model_kwargs)
                            # 恢复初始 attention mask 并更新当前块的 mask
                            # model_inputs["attention_mask"] = attention_mask_iteration.clone()
                            model_inputs_chunk["attention_mask"][0].fill_(1)
                            model_inputs_chunk["attention_mask"][0, start:end] = 0.0
                    ...
                    if first_iteration:
                        ...
                        for i in range(len(self.logits_chunks)):
                            self.w_list.append(calculate_kl_divergence(self.logits_chunks[i].squeeze(0), next_token_logits_base.squeeze(0)))
                    ...
                    imb_factor = 0.001  # 可调整imb_factor值以改变分布的陡峭程度
                    # 生成长尾分布列表
                    ...
                    next_token_scores = weighted_logits_sum / sum(self.long_tail_weights)
                    ...
                    for i in self.topk_indices:
                        # 只对选出来的topk元素进行计算
                        calibrated_logits = (1 + 1.5) * next_token_logits - 1.5 * self.logits_chunks[i]
                        self.logits_list.append(calibrated_logits)
                    # 使用long_tail_weights对logits_list进行加权计算
                    weighted_logits_sum = sum(logit * weight for logit, weight in zip(self.logits_list[0:cali_top_k], self.long_tail_weights[0:cali_top_k]))
                    next_token_scores = weighted_logits_sum / sum(self.long_tail_weights)
